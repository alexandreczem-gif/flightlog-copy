...
export default function VictimRecordForm({ initialData, onSave, isSaving }) {
    ...
    const validateForm = () => {
        ...
        // Validação dos Dados do Transporte
        if (!data.tipo_transporte) {
            errors.tipo_transporte = "Tipo de Transporte é obrigatório";
        }
        if (!data.status_transporte) {
            errors.status_transporte = "Status do Transporte é obrigatório";
        }
        ...
        if (!data.suporte_ventilatorio) {
            errors.suporte_ventilatorio = "Suporte Ventilatório é obrigatório";
        }
        if (!data.uso_sedacao) {
            errors.uso_sedacao = "Uso de Sedação é obrigatório";
        }
        if (!data.uso_droga_vasoativa) {
            errors.uso_droga_vasoativa = "Uso de Droga Vasoativa é obrigatório";
        }
        if (!data.glasgow || data.glasgow === '') {
            errors.glasgow = "Glasgow é obrigatório";
        }
        if (!data.transfusao) {
            errors.transfusao = "Transfusão é obrigatório";
        }
        ...
        if (!data.compressor_toracico) {
            errors.compressor_toracico = "Compressor Torácico é obrigatório";
        }
        if (!data.ultrassom_portatil) {
            errors.ultrassom_portatil = "Ultrassom Portátil é obrigatório";
        }
        if (!data.babypod) {
            errors.babypod = "Babypod é obrigatório";
        }
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <FormSection title="Dados Administrativos (Importado)">
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div><Label>Ano</Label><Input value={data.ano} readOnly className="bg-slate-100" /></div>
                    <div><Label>Mês</Label><Input value={data.mes} readOnly className="bg-slate-100" /></div>
                    <div><Label>Data</Label><Input value={data.data ? (() => {
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div>
                        ...
                        <Select 
                            ...
                            onValueChange={v => handleChange('tipo_transporte', v)}
                        >
                        ...
                        {validationErrors.tipo_transporte && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.tipo_transporte}</p>
                        )}
                    </div>
                    <div>
                        <Label htmlFor="status_transporte">Status do Transporte <span className="text-red-500">*</span></Label>
                        <Select 
                            value={data.status_transporte} 
                            onValueChange={v => handleChange('status_transporte', v)}
                        >
                        ...
                        {validationErrors.status_transporte && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.status_transporte}</p>
                        )}
                    </div>
                    {data.status_transporte === 'Cancelado' && (
                        <div>
                            ...
                            <Select 
                                value={data.motivo_qta} 
                                onValueChange={v => handleChange('motivo_qta', v)}
                            >
                            ...
                            {validationErrors.motivo_qta && (
                                <p className="text-xs text-red-500 mt-1">{validationErrors.motivo_qta}</p>
                            )}
                        </div>
                    )}
                </div>
            ...
            <FormSection title="Dados do Paciente (Importado)">
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="lg:col-span-2"><Label>Nome</Label><Input value={data.nome_paciente} readOnly className="bg-slate-100" /></div>
                    <div><Label>Sexo</Label><Select value={data.sexo_paciente} disabled><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{SEXO_OPTIONS.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select></div>
                    <div><Label>Idade</Label><Input value={data.idade} readOnly className="bg-slate-100" /></div>
                    <div className="lg:col-span-2"><Label>Faixa Etária</Label><Select value={data.faixa_etaria} disabled><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{FAIXA_ETARIA_OPTIONS.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select></div>
                </div>
            </FormSection>
            ...
            <FormSection title="Dados Clínicos">
                 <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        {validationErrors.grupo_patologias && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.grupo_patologias}</p>
                        )}
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        {validationErrors.suporte_ventilatorio && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.suporte_ventilatorio}</p>
                        )}
                    </div>
                    <div>
                        ...
                        {validationErrors.uso_sedacao && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.uso_sedacao}</p>
                        )}
                    </div>
                    <div>
                        ...
                        {validationErrors.uso_droga_vasoativa && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.uso_droga_vasoativa}</p>
                        )}
                    </div>
                    <div>
                        ...
                        />
                        {validationErrors.glasgow && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.glasgow}</p>
                        )}
                    </div>
                    <div>
                        ...
                        {validationErrors.transfusao && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.transfusao}</p>
                        )}
                    </div>
                    {data.transfusao === 'Sim' && (
                        <div>
                            ...
                            />
                            {validationErrors.transfusao_bolsas && (
                                <p className="text-xs text-red-500 mt-1">{validationErrors.transfusao_bolsas}</p>
                            )}
                        </div>
                    )}
                    <div>
                        ...
                        {validationErrors.pressao_arterial_sistolica && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.pressao_arterial_sistolica}</p>
                        )}
                    </div>
                    <div>
                        ...
                        {validationErrors.frequencia_cardiaca && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.frequencia_cardiaca}</p>
                        )}
                    </div>
                    <div>
            ...
            <FormSection title="Dados da Missão (Importado)">
                 <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div><Label>Cidade Origem</Label><CityCombobox value={data.cidade_origem} disabled={true} options={CITY_OPTIONS} /></div>
                    <div>
                        <Label>Hospital Origem</Label>
                        <div className="space-y-2">
                            <HospitalCombobox 
                                value={data.hospital_origem} 
                                disabled={true}
                                cityFilter={data.cidade_origem}
                            />
                            <div className="flex items-center gap-2">
                                <Checkbox 
                                    checked={data.hospital_origem === 'Não se aplica'}
                                    disabled={true}
                                />
                                <Label className="text-sm font-normal">Não se aplica</Label>
                            </div>
                        </div>
                    </div>
                    <div>
                        <Label>Local Pouso Origem</Label>
                        <Select value={data.local_pouso_origem || ''} disabled={true}>
                    ...
                    <div><Label>Cidade Destino</Label><CityCombobox value={data.cidade_destino} disabled={true} options={CITY_OPTIONS} /></div>
                    <div>
                        <Label>Hospital Destino</Label>
                        <div className="space-y-2">
                            <HospitalCombobox 
                                value={data.hospital_destino} 
                                disabled={true}
                                cityFilter={data.cidade_destino}
                            />
                            <div className="flex items-center gap-2">
                                <Checkbox 
                                    checked={data.hospital_destino === 'Não se aplica'}
                                    disabled={true}
                                />
                                <Label className="text-sm font-normal">Não se aplica</Label>
                            </div>
                    ...
                    <div>
                        <Label>Local Pouso Destino</Label>
                        <Select value={data.local_pouso_destino || ''} disabled={true}>
                            <SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger>
                            <SelectContent>
                                <SelectItem value="Heliponto">Heliponto</SelectItem>
                                <SelectItem value="Aeródromo">Aeródromo</SelectItem>
                ...
                <div className="mt-6">
                    <h4 className="font-medium text-slate-700 mb-4">Etapas de Voo (Horários Z)</h4>
                    <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-4">
                        {[1, 2, 3, 4, 5].map(stage => (
                            <div key={stage} className="space-y-2 p-3 bg-slate-50 rounded-lg">
                                <p className="text-xs font-semibold text-slate-600">Etapa {stage}</p>
                                <div>
                                    <Label className="text-xs">Decolagem</Label>
                                    <Input 
                                        type="time" 
                                        value={data[`departure_time_${stage}`] || ''} 
                                        readOnly
                                ...
                                <div>
                                    ...
                                    <Input 
                                        type="time" 
                                        value={data[`arrival_time_${stage}`] || ''} 
                                        readOnly
                                        className="text-sm bg-slate-100"
                                    />
                                </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                    <div><Label>Duração Total (min)</Label><Input type="number" value={data.duracao_total_min} readOnly className="bg-slate-100" /></div>
                    <div><Label>Aeronave</Label><Input value={data.aeronave} readOnly className="bg-slate-100" /></div>
                    <div><Label>Nº Pág. Diário de Bordo</Label><Input value={data.diario_bordo_pagina} readOnly className="bg-slate-100" /></div>
            ...
            <FormSection title="Equipamentos Utilizados">
                <div className="grid md:grid-cols-3 gap-6">
                    <div>
                        ...
                        {validationErrors.compressor_toracico && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.compressor_toracico}</p>
                        )}
                    </div>
                    <div>
                        ...
                        {validationErrors.ultrassom_portatil && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.ultrassom_portatil}</p>
                        )}
                    </div>
                    <div>
                        ...
                        {validationErrors.babypod && (
                            <p className="text-xs text-red-500 mt-1">{validationErrors.babypod}</p>
                        )}
                    </div>
                </div>
            </FormSection>
